...
## format raster
historical_avg_crs <- terra::project(historical_avg, ocean, method="bilinear")
historical_avg_crs[is.nan(historical_avg_crs)] <- NA

# gapfill coastline
historical_avg_crs_gf <- historical_avg_crs
for(i in 1:200){
  i=1+i
historical_avg_crs_gf <- terra::focal(historical_avg_crs_gf, w=3, fun=mean, na.rm=TRUE, na.policy="only")
}

historical <- terra::mask(historical_avg_crs_gf, ocean)

writeRaster(historical, "/home/shares/ohi/stressors_2021/_dataprep/oa/raw/historical.tif", overwrite=TRUE)

# check to see how much is <1
tmp <- terra::ifel(historical <= 1, 1, NA)
plot(tmp, col="red")

tmp <- terra::ifel(historical >3, 1, NA)
plot(tmp, col="red")
```
...
#stack_list=stack_list_spp, historical_avg=historical
...
## format raster
oa_crs <- terra::project(oa_predict, ocean, method="bilinear")
oa_crs[is.nan(oa_crs)] <- NA

# gapfill coastline
oa_crs_gf <- oa_crs
for(i in 1:200){
  i=1+i
oa_crs_gf <- terra::focal(oa_crs_gf, w=3, fun=mean, na.rm=TRUE, na.policy="only")
}


oa_predict <- terra::mask(oa_crs_gf, ocean)

terra::writeRaster(oa_predict, filename=sprintf("/home/shares/ohi/stressors_2021/_dataprep/oa/raw/raw_oa_%s_%s.tif",
...
terra::writeRaster(oa_pressure, filename=sprintf("/home/shares/ohi/stressors_2021/_dataprep/oa/rescaled/oa_%s_%s.tif",
              saveName, select_time_frame), overwrite=TRUE)
}
}
```
...
old <- rast("/home/shares/ohi/stressors_2021/_dataprep/oa/oa_ssp245_medium-term.tif")
new <- rast("/home/shares/ohi/stressors_2021/_dataprep/oa/rescaled/oa_ssp245_medium-term.tif")
plot(old - new)
tmp <- old-new
click(tmp)
tmp <- ifel(tmp!=0, 1, 0)
