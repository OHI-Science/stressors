...
## rescaling and masking the data!
```{r}

## get mask that includes coastline and one cell offshore
## prepare raster mask
...
path <- "/home/shares/ohi/stressors_2021/_dataprep/population_v2/pop_density_km2_ocean_expand_log_rescaled_crop/"
orgs <- grep("SSP1", list.files(path, full=TRUE), value=TRUE)

# Create vector of new names
new_files <- c(
 gsub("SSP1", "ssp126", orgs),
 gsub("SSP1", "ssp119", orgs)
)


# Copy files with new names
file.copy(orgs, new_files)

orgs <- grep("SSP", list.files(path, full=TRUE), value=TRUE)
...
for(old_pattern in names(rename_patterns)) { # old_pattern <- names(rename_patterns)[1]
 files_to_rename <- grep(old_pattern, orgs, value=TRUE)
 new_names <- gsub(old_pattern, rename_patterns[old_pattern], files_to_rename)
