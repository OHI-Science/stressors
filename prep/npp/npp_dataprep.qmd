...
**Caption:** Primary Organic Carbon Production by All Types of Phytoplankton for ENSMN interpolated on a 1x1 grid for the entire year; **First panel:**  mean climate from the historical experiment for the period 1955-1984; **Second panel:** difference in the mean climate in the future time period (e.g., ssp126: 2020-2049) compared to the historical reference period (1955-1984); **Third panel:** inter-annual (de-trended) standard deviation for the historical reference period (1955-1984); **Fourth panel:** ratio of the de-trended variance in the future (e.g., 2020-2049) divided by the past (1955-1984).

The current period is from 1985-2014.
...
```{r}
tmp1 <- terra::rast("/home/shares/ohi/stressors_2021/_raw_data/npp_noaa/models_hist_1955-1984/npp_ssp126_2020_2049_v1.nc")
test1 <- tmp1[[1]]
test3 <- tmp1[[3]]

tmp2 <- terra::rast("/home/shares/ohi/stressors_2021/_raw_data/npp_noaa/models_hist_1955-1984/npp_ssp126_2040_2069_v1.nc")
test2 <- tmp2[[1]]

plot(test2-test1) # good! these are the same data, which makes sense (but I wanted to check)


tmp <- nc_open("/home/shares/ohi/stressors_2021/_raw_data/npp_noaa/models_hist_1955-1984/npp_ssp126_2020_2049_v1.nc")
names(tmp[['var']])
...
nc_files <- list.files("/home/shares/ohi/stressors_2021/_raw_data/npp_noaa/models_hist_1955-1984/", full=TRUE)
...
file_names <- nc_open(nc)
 ...
 fileName_future <- gsub("-\\(1955-1984\\)", "", fileName_future)
...
tmp <- terra::rast("/home/shares/ohi/stressors_2021/_raw_data/npp_noaa/models_hist_1955-1984/npp_ssp126_2020_2049_v1.nc")
terra::plot(tmp[[2]])
...
Get the "historical" from 1984-2014
...
file_names <- nc_open(nc)
 ...
 fileName_his <- ncatt_get(file_names, attributes(file_names$var)$names[layer+n_layers])$info
 fileName_his <- sub("anomaly \\(.*\\)", "current", fileName_his)
 fileName_his <- gsub(" ", "_", fileName_his)
...
ensemble_combos <- expand.grid(scenario=c("ssp126", "ssp245", "ssp370", "ssp585"),
            time = c("current", "historical", "(2020-2049)", "(2040-2069)", "(2070-2099)"))
...
for(scenario in scenarios){ #scenario = scenarios[1]
  ...
  current <- rast(grep("current", scenario_rasters, value=TRUE))
  ...
  long <- rast(grep("2070-2099", scenario_rasters, value=TRUE))
    ...
    writeRaster(current, sprintf("/home/shares/ohi/stressors_2021/_dataprep/npp/raw_rescaled/npp_raw_%s_current.tif", scenario), overwrite=TRUE)
  writeRaster(historical, sprintf("/home/shares/ohi/stressors_2021/_dataprep/npp/raw_rescaled/npp_raw_%s_historical.tif", scenario), overwrite=TRUE)
  ...
  current_rescaled <- 1 - current/historical
  current_rescaled <- ifel(current_rescaled>1, 1, current_rescaled)
  current_rescaled <- ifel(current_rescaled<0, 0, current_rescaled)      
  ...
  long_rescaled <- ifel(long_rescaled<0, 0, long_rescaled)  
      ...
      writeRaster(current_rescaled, sprintf("/home/shares/ohi/stressors_2021/_dataprep/npp/raw_rescaled/npp_rescaled_%s_current.tif", scenario), overwrite=TRUE)
