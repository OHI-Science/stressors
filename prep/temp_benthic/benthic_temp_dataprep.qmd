...
temp_paths <- list.files("/home/shares/ohi/stressors_2021/_raw_data/biooracle_temperature/", full=TRUE)

for(temp_path in temp_paths){ # temp_path = temp_paths[1]
temp_rast <- terra::rast(temp_path)

temp_rast <- terra::project(temp_rast, rast_base_10km_file, method="bilinear", over=TRUE)
#plot(temp_rast)
...
# plot(r)
r <- r*ocean_template
# diff_rast <- r - temp_rast
#plot(diff_rast) # should be 0 

saveName <- basename(temp_path)

writeRaster(r, filename = sprintf("/home/shares/ohi/stressors_2021/_dataprep/T_benthic/projected/%s", saveName), overwrite = TRUE)
}
...
rescale_temp <- function(scenario = "ssp585",
                    ...
                    historic_range = "/home/shares/ohi/stressors_2021/_dataprep/T_benthic/projected/thetao-depthmean_range_hist_hist.tif",
                    historic_mean = "/home/shares/ohi/stressors_2021/_dataprep/T_benthic/projected/thetao-depthmean_mean_hist_hist.tif"){

    hist_mean <- rast(historic_mean)
    hist_range <- rast(historic_range)
  
future <- rast(sprintf("/home/shares/ohi/stressors_2021/_dataprep/T_benthic/projected/thetao-depthmean_mean_%s_%s.tif", scenario, time_frame))
...
```{r}
new <- rast("/home/shares/ohi/stressors_2021/_dataprep/T_benthic/rescaled/benthic-temp_ssp245_medium-term.tif")
old <- rast("/home/shares/ohi/stressors_2021/_dataprep/T_benthic/rescaled_mol/benthic-temp_ssp245_medium-term.tif")
