...
The Oxygen data are bio-oracle data from here:
...
list_layers("Oxygen")$dataset_id
dataset_id <- "o2_ssp245_2020_2100_depthsurf"
...
writeRaster(layers, filename = file.path("//home/shares/ohi/stressors_2021/_raw_data/biooracle_oxygen", paste0(variable, "-", depth, "_hist_hist.tif")), overwrite=TRUE)
...
writeRaster(layers, filename = file.path("/home/shares/ohi/stressors_2021/_raw_data/biooracle_oxygen", paste0(variable, "-", depth, "_current_current.tif")), overwrite=TRUE)
...
writeRaster(rmean, filename = file.path("/home/shares/ohi/stressors_2021/_raw_data/biooracle_oxygen", paste0(variable, "-", depth, "_", scenario, "_near-term.tif")), overwrite=TRUE)
  
##medium
time = c('2040-01-01T00:00:00Z', '2050-01-01T00:00:00Z')
layers <- layer_download(time=time, latitude=latitude, longitude = longitude, dataset_id = dataset_id)
rmean <- app(layers, mean)
writeRaster(rmean, filename = file.path("/home/shares/ohi/stressors_2021/_raw_data/biooracle_oxygen", paste0(variable, "-", depth, "_", scenario, "_medium-term.tif")), overwrite=TRUE)

##long
time = c('2080-01-01T00:00:00Z', '2090-01-01T00:00:00Z')
layers <- layer_download(time=time, latitude=latitude, longitude = longitude, dataset_id = dataset_id)
rmean <- app(layers, mean)
writeRaster(rmean, filename = file.path("/home/shares/ohi/stressors_2021/_raw_data/biooracle_oxygen", paste0(variable, "-", depth, "_", scenario, "_long-term.tif")), overwrite=TRUE)
...
oxy_paths <- list.files("/home/shares/ohi/stressors_2021/_raw_data/biooracle_oxygen", full=TRUE)
...
saveName <- basename(oxy_path)

writeRaster(r, filename = sprintf("/home/shares/ohi/stressors_2021/_dataprep/oxygen/projected/%s", basename(oxy_path)), overwrite = TRUE)
}
...
files2scale <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/projected", pattern = "depthmean", full=TRUE)

for(file2scale in files2scale){ # file2scale <- files2scale[1]
historic_rast <- rast("/home/shares/ohi/stressors_2021/_dataprep/oxygen/projected/o2-depthmean_hist_hist.tif")
...
# surface water
files2scale <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/projected", pattern = "depthsurf", full=TRUE)

for(file2scale in files2scale){ # file2scale <- files2scale[1]
historic_rast <- rast("/home/shares/ohi/stressors_2021/_dataprep/oxygen/projected/o2-depthsurf_hist_hist.tif")
stress_rescale <- o2rescale(fp_current = file2scale, 
          historic = historic_rast)
writeRaster(stress_rescale, file.path("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled", basename(file2scale)), overwrite=TRUE)
}
...
# delete the historical ones
file_list <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled", full=TRUE)
delete_files <- grep("hist", file_list, value=TRUE)
file.remove(delete_files)

# expand the current ones
scenarios <- c("ssp119", "ssp126", "ssp245", "ssp370", "ssp460", "ssp585")
file_list <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled", full=TRUE)
current_files <- grep("current", file_list, value=TRUE)
...
for(current_file in current_files){ #current_file <- current_files[1]
  ...
  writeRaster(tmp, sprintf("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled/%s", saveName))
  }}

# delete the current ones
file_list <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled", full=TRUE)
delete_files <- grep("current_current", file_list, value=TRUE)
file.remove(delete_files)
...
Quick look at some!
```{r}
old <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled_old", full=TRUE)
new <- list.files("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled", full=TRUE)


tmp <- rast("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled_mol/o2-depthmean_ssp245_medium-term.tif") - rast("/home/shares/ohi/stressors_2021/_dataprep/oxygen/rescaled/o2-depthmean_ssp245_medium-term.tif")
plot(tmp)
tmp <- ifel(tmp>0.2, 1, NA)
plot(tmp, col="red")

plot(new[10])
tmp <- rast(new[10]) - rast(new[1])
