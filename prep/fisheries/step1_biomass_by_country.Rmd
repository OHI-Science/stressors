...
## explore USA
ggplot(filter(country_biomass_change, rgn_nam == "United States"), aes(x=year, y=biomass)) +
  geom_point() + 
  geom_line()
ggplot(filter(country_biomass_change, rgn_nam == "United States"), aes(x=year, y=yearly_percent_change)) +
  geom_point() + 
  geom_line()

summary(lm(yearly_percent_change ~ year, data=filter(country_biomass_change)))
...
country_avg_change <- country_biomass_change %>%
  ...
  select(rgn_id, ant_typ, rgn_nam, iso3c, uninhabited, avg_biomass, avg_percent_change)
...
#  left_join(country_avg_change2, by=c("rgn_id", "ant_typ", "rgn_nam")) %>%
  select(rgn_id, ant_typ, rgn_nam, iso3c, uninhabited, avg_biomass, avg_percent_change, slope=estimate)
...
## add in most recent biomass
biomass_historic_times <- which(as.Date(time(biomass)) == as.Date("2010-01-01")) 
biomass_h <- subset(biomass, biomass_historic_times)
biomass_h <- cellSize(biomass_h) * biomass_h * 1e-6
# extract current yearly biomass for each eez
country_current_biomass <- terra::zonal(biomass_h, zones_raster, fun = sum, na.rm=TRUE)

names(country_current_biomass) <- c("rgn_id", "current_biomass")

country_current_avg <- left_join(country_avg_change, country_current_biomass, by="rgn_id")

## save data
write_csv(country_current_avg, here("prep/fisheries/data/region_biomass_trends.csv"))
