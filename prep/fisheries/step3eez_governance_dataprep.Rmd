...
gov_gf_avg2 <- left_join(gov_gf_avg, gdp_pc, by="iso3c")  # N=37 of 61 have gdp data
...
# a few countries don't have all years of gdp data, gapfill here:
gdp_pc <- gdp_pc %>%
  select(iso3c, year, pcGDP=NY.GDP.PCAP.PP.CD) %>%
  group_by(iso3c) %>%
  arrange(year) %>%
...
gov_gf2 <- left_join(gov_gf, gdp_pc, by=c("iso3c", "year"))  # N=23 of 61 have gdp data



# Add in the values I looked up from other sources
## rough estimate from google searches (Wikipedia and such)
gf_regions <- read_csv(here("prep/fisheries/data/country_pop_gf.csv")) %>%
  filter(population > 0) %>%
  mutate(pop_gf = population/1000000,
...
no_data_ids <- setdiff(governance_gf_2010_2015$iso3c, bias_corrected_future$iso3c)

scenario = c("SSP1", "SSP2", "SSP3", "SSP4", "SSP5")
governance_2015 <- filter(governance_gf_2010_2015, iso3c %in% no_data_ids) %>%
  filter(year == 2015) %>%
