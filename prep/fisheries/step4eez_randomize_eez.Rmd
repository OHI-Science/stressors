...
simulated_data_tmp <- as.data.frame(t(expected_slopes))
...
update_biomass <- function(df, random_column) {
  ...
  for (i in 2:nrow(df)) { # i=2
    ...
    new_percent_change <- df$avg_percent_change[i - 1] + df[[random_column]][i - 1] 
...
usa_biomass_stats <- data_long %>% # tza, brb ,chn
  filter(iso3c=="CHN") %>%
...
ggplot() + 
  geom_line(data=filter(simulation, scenario == "SSP2"), aes(x=year, y=total_biomass, group=biomass_type), alpha=0.2) +
