...
Use historical data to determine relative change:
```{r}
...
plot(mask)
mask_latlon <- project(mask, "EPSG:4236")
NA_extent <- ext(-170, -50, 5, 85) 
mask_latlon <- crop(mask_latlon, NA_extent)
pal <- leaflet::colorNumeric(palette = "viridis", domain = values(mask_latlon), alpha = TRUE)
mapview::mapview(mask_latlon, maxpixels=970830, col.regions=pal)
leaflet() %>%
  addTiles() %>%
  addRasterImage(mask_latlon, colors = pal, opacity = 0.5) 
