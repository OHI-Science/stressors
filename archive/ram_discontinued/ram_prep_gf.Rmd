...
## ok these are all unique:
check <- paste(ram_biomass$stockid, ram_biomass$year)
length(unique(check))
length(check)
...
#collect data and eliminate stocks with < 10 years of data
ram_biomass_gf <- ram_biomass %>%
  group_by(stockid) %>%
...
## Expand data to include all years
ram_biomass_gf <- ram_biomass_gf %>%
...
## gapfill. missing middle values interpolated 
## first missing values, value of first non missing
## last missing values, value of last non missing
ram_biomass_gf <- ram_biomass_gf %>%
  arrange(stockid, year) %>%
  mutate(biomass_MT = na.approx(biomass_MT, na.rm = FALSE, rule = 2)) %>%
  ungroup() 

table(ram_biomass_gf$stockid)


# cut ones that haven't been assessed since 2010
# cut ones that weren't assessed until after 2000
ram_biomass_gf2 <- ram_biomass_gf %>%
  filter(max_year >= 2010) %>%
  filter(min_year <= 2000)

write_csv(ram_biomass_gf2, here("prep/fisheries/data/RAM_gapfilled.csv"))
